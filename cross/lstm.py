...
def plot_wrapper(inp, out, plots, directory, fname, tokenizer):
    ...
    output = output[0][:output[0].find('>')+1:2]

    # Hotfix: the attention plot generated by train step contains an initial
    # row of zeros
    plot_attention(char_plot[1:len(output)+1, :len(char_input)],
                   [x for x in char_input], [x for x in output],
                   os.path.join(directory, fname + '-enc.png'))
    plot_attention(tag_plot[1:len(output)+1, :len(tag_input)],
